...
<body class="bg-slate-50 font-sans antialiased min-h-screen flex flex-col justify-between relative overflow-hidden">
    ...
    <main class="flex-1 flex flex-col justify-center items-center p-6 my-4 relative z-10">
        ...
        <div class="max-w-md w-full bg-white rounded-2xl border border-gray-100 shadow-xl p-8 space-y-6">
            ...
            @if (session('status'))
                <div
                    class="bg-emerald-50 border border-emerald-200 text-emerald-600 px-4 py-2 rounded-lg text-xs font-medium">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-4">
                ...
                <div class="space-y-1">
                    <div class="flex justify-between items-center">
                        <label for="password"
                            class="text-xs font-bold text-gray-700 tracking-wide block">Password</label>
                ...
                <div class="flex items-center">
                    ...
                    <label for="remember_me" class="ms-2 text-xs text-gray-600 select-none cursor-pointer">Remember
                        me</label>
                </div>
